<template>
  <div class="h-screen w-64 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-700 p-4 flex flex-col">
    ...
    <!-- Меню -->
    <Menu :model="items" class="w-full mt-2 border-none shadow-none bg-transparent dark:bg-transparent">

      <!-- Заголовок подменю -->
      <template #submenulabel="{ label }">
        <span class="text-sm font-bold text-gray-500 dark:text-gray-400">{{ label }}</span>
      </template>

      <!-- Пункт меню -->
      <template #item="{ item, props }">
        <a v-if="item.to" :href="props.action.href" v-ripple class="flex items-center px-3 py-2 rounded hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors cursor-pointer"
        ...
        <a v-else v-ripple class="flex items-center px-3 py-2 rounded hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors cursor-pointer"
    ...
    <div class="mt-auto pt-4 border-t border-gray-200 dark:border-gray-700">
      <button class="w-full flex items-center p-2 rounded hover:bg-gray-100 dark:hover:bg-gray-800">
        <img src="https://primefaces.org/cdn/primevue/images/avatar/amyelsner.png " alt="Profile" class="w-8 h-8 rounded-full mr-3" />
        <div class="text-left">
          <div class="font-bold text-sm">Amy Elsner</div>
    ...
    if (item.items) {
      return {
        ...
        items: item.items.map(subItem => ({
          ...subItem,
          class: route.path === subItem.to ? 'active' : ''
        }))
      }
    }
    return {
